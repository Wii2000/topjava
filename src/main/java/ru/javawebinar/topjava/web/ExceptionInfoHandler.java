...
public class ExceptionInfoHandler {
    ...
    private static final Map<String, String> CONSTRAINS_I18N_MAP = Map.of(
            "users_unique_email_idx", "error.emailDuplication",
            "meals_unique_user_datetime_idx", "error.dateTimeDuplicate");
    ...
    public ErrorInfo conflict(HttpServletRequest req, DataIntegrityViolationException e) {
        ...
        if (rootMsg != null) {
            String msg = rootMsg.toLowerCase();
            for (Map.Entry<String, String> entry : CONSTRAINS_I18N_MAP.entrySet()) {
                if (msg.contains(entry.getKey())) {
                    errorMsg = messageSource.getMessage(entry.getValue(), null, req.getLocale());
                }
            }
        }

        return logAndGetErrorInfo(req, e, true, DATA_ERROR, errorMsg);
    }
    ...
    public ErrorInfo handleMethodArgumentNotValidException(HttpServletRequest req, MethodArgumentNotValidException e) {
        return logAndGetErrorInfo(req, e, false, VALIDATION_ERROR, getFieldErrors(e.getBindingResult()));
    }
